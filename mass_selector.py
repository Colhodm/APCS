	...
	for i in range(len(inputs)):
		...
		with open(inputs[i], 'r') as csvfile:
			...
			for row in reader:
				if not row:
					break
				accession_ids.append((row[0], row[1], i))

	# Shuffle accession IDs
	random.shuffle(accession_ids)
	mass_count = 0
	tot_count = 0

	print "Extracting {} masses from a total of {} accession IDs".format(n, len(accession_ids))

	# Write results
	for acc_id, text, src_idx in accession_ids:
		if mass_count >= n:
			break
		tot_count += 1

		if process_entry(acc_id, text, 0, nlp_path):
			if nlp_result_has_mass(nlp_path + '/' + row[0] + '_selen.txt'):
				mass_count += 1
