...
#input_filename = "ALLREALMALIGNANTS - Sheet1.csv"
input_filename = "ALLREALBENIGN - TrainingSET.csv"
#input_filename = "RNNBENIGN.csv - Sheet 1.csv"
#input_filename = "Markov-Benign-Final.csv - Markov-Benign-Final.csv.csv"

for x in xrange(numberofclusters):
  output_filename = "REALPLUSREAL25%" + str(x) + ".csv"
  output_filenameother = "REALPLUSREAL25TESTING%" + str(x) + ".csv"
  output_line = ""
...
# Skip the first line of the CSV file because it's the header, which we don't need
  ...
  for x in xrange(sizeoftestingset):
      randomarray[x] = random.randint(2, 13343)
          ...
          #        15] + "," + input_line[16] + "," + input_line[17] + "," + input_line[18] + "," + \
           #   input_line[19] + "," + input_line[20] + "," + input_line[21] + "," + input_line[
            #      22] + "," + input_line[23] + "," + input_line[24] + "," + input_line[25] + "," + \
             ...
             #     36]
            #output_fileother.write('\n')


      #for y in range(sizeoftestingset):
      if (randomarray[0] == linecounter or randomarray[1] == linecounter or randomarray[2] == linecounter or
                  randomarray[3] == linecounter or randomarray[4] == linecounter or randomarray[5] == linecounter or
                  randomarray[6] == linecounter or randomarray[7] == linecounter or randomarray[8] == linecounter or
                  randomarray[9] == linecounter):
          ...
          #     print(randomarray[y])
                ...
                if(input_line[0] == benignidentifier and benigncounter < numberofrecords):
...
# Close the files after we're done with them so other programs can use them.
  ...
  if(benigncounter == numberofrecords):
    output_file.close()
